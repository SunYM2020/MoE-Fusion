...
class M3FD_TSCustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_infos, self.rgb_img_infos= self.load_annotations(ann_file)       # loda tir and rgb image
    ...
    def prepare_train_img(self, idx):
        ...
        img_r = mmcv.imread(osp.join(self.img_prefix, 'Vis', img_info_r['filename']))     # Replace 'Vis' with YOUR VISIBLE DIR
        img_i = mmcv.imread(osp.join(self.img_prefix, 'Ir', img_info_i['filename']))      # Replace 'Ir' with YOUR INFRARED DIR
        ...
        img_r = mmcv.imresize(img_r, (640, 480), return_scale=False)
        img_i = mmcv.imresize(img_i, (640, 480), return_scale=False)
 
        source_img_r = img_gray
        source_img_r = mmcv.imresize(source_img_r, (640, 480), return_scale=False)
        source_img_r = source_img_r[:,:,np.newaxis].astype(np.float32)

        source_img_i = img_i[:,:,0]
        source_img_i = mmcv.imresize(source_img_i, (640, 480), return_scale=False) 
