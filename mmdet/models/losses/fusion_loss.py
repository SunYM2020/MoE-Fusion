...
class NewGradLoss(nn.Module):
    """Loss function for cross modal using grad loss.
    ...
    def __init__(self, loss_weight=1.0):
        super(NewGradLoss, self).__init__()
...
class DetcropPixelLoss(nn.Module):
    """Loss function for the pixcel loss between max(RGB,TIR) and Fusion images.
        ...
        for k in range(len(detect_box)):
            if k == len(detect_box)-1:
                for i in range(len(detect_box[k])):
                    ...
                    if int(detect_box[k][i][1])>480:
                        detect_box[k][i][1]=480
                    if int(detect_box[k][i][3])>480:
                        detect_box[k][i][3]=480
