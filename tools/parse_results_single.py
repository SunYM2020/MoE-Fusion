...
def parse_results(config_file, resultfile, dstpath, type):
    ...
    if type == 'OBB':
        #  dota1 has tested
        #obb_results_dict = OBBDetComp4_rgb(dataset, outputs)
        obb_results_dict = OBBDetComp4_infrared(dataset, outputs)
...
if __name__ == '__main__':
    ...
    config_name = 'DSUADet_roitrans_DSloss_2x_1gpu_4pic_0.005'
    pkl_file = os.path.join('work_dirs', config_name, 'results.pkl')       # 1 stream
    output_path = os.path.join('work_dirs', config_name)
    type = args.type
    parse_results(config_file, pkl_file, output_path, type)                # 1 stream
